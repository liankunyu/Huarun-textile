...
var
  frmzfpzfgldsh: Tfrmzfpzfgldsh;

implementation
uses UDM,zfpzfg;
{$R *.dfm}

procedure Tfrmzfpzfgldsh.btnExitClick(Sender: TObject);
begin
Close;
end;

procedure Tfrmzfpzfgldsh.btnExamineClick(Sender: TObject);
begin
  ...
  begin
    try
      ...
      begin
        Close;
        procedureName:='proc_自纺通知单领导审核_审核状态更新';
        Parameters.Refresh;
        Parameters.ParamByName('@bh').Value:=DM.qryzfdldsh.FieldByName('编号').AsString;
        Parameters.ParamByName('@ldshr').Value:=frmzfpzfg.User;
        ExecProc;
