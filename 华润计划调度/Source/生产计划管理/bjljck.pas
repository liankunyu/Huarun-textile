...
uses
  ...
  cxContainer, cxEdit, dxSkinsCore, dxSkinsDefaultPainters, RzTabs,
  cxLabel, StdCtrls, Mask, RzEdit, RzLabel, RzPanel, RzButton, ExtCtrls,
  ImgList, ComCtrls,Math;
...
type
  TFrmBJLJCK = class(TForm)
    ...
    cxlblPlanMachineType: TcxLabel;
    cxlblProductName: TcxLabel;
...
begin
 with DM.qryBShutDownDaysQuery do
  ...
  cxlblProductName.Visible:=True;
  cxlblProductName.Caption:='品名:'+DM.qryBProductionPlan.FieldByName('品名').AsString;
  cxlblCustomerName.Visible:=True;
  cxlblCustomerName.Caption:='客户名称:'+DM.qryBProductionPlan.FieldByName('客户名称').AsString;
  cxlblPlanMachineType.Visible:=True;
  cxlblPlanMachineType.Caption:='计划机型:'+DM.qryBProductionPlan.FieldByName('计划机型').AsString;
end;

procedure TFrmBJLJCK.btnSchedulingClick(Sender: TObject);
begin
  FrmDDXXLR:=TFrmDDXXLR.Create(Self);
  FrmDDXXLR.ShowModal;
  FreeAndNil(FrmDDXXLR);
end;
...
procedure TFrmBJLJCK.RzToolButton1Click(Sender: TObject);
var
  i,j:Integer;
begin
   with DM.qryBShutDownDaysQuery do
...
procedure TFrmBJLJCK.RzToolButton2Click(Sender: TObject);
var
  j:Integer;
begin
     for j:=0 to Self.ComponentCount-1 do
...
procedure TFrmBJLJCK.RzToolButton4Click(Sender: TObject);
var
  i,j:Integer;
begin
