...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select * from 机台了机信息 where 预了机时间 between :ylzrq1 and :ylzrq2 ');
 ...
 with DM.qry3 do
  begin
    Close;
    SQL.Clear;
    SQL.Add('select count(*) as record_count from 机台了机信息 where 了机后状态=:ljhzt and 预了机时间 between :ylzrq1 and :ylzrq2');
 ...
 with DM.qry4 do
  begin
    Close;
    SQL.Clear;
    SQL.Add('select count(*) as record_count1 from 机台了机信息 where 了机后状态=:ljhzt and 预了机时间 between :ylzrq1 and :ylzrq2');
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select * from 机台了机信息 where 品名 is null');
    Open;
  end;
  if DM.qryBShutDownDaysQuery.Eof  then
  begin
...
begin
 with DM.qry3 do
  begin
    Close;
    SQL.Clear;
    SQL.Add('select count(*) as record_count from 机台了机信息 where 了机后状态=:ljhzt');
    Parameters.ParamByName('ljhzt').Value:='上轴';
    Open;
  end;
 edt1.Text:=DM.qry3.FieldByName('record_count').AsString;
 with DM.qry4 do
  begin
    Close;
    SQL.Clear;
    SQL.Add('select count(*) as record_count1 from 机台了机信息 where 了机后状态=:ljhzt');
    Parameters.ParamByName('ljhzt').Value:='结经';
    Open;
  end;
 edt2.Text:=DM.qry4.FieldByName('record_count1').AsString;
